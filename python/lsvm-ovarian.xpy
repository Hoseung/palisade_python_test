...
with open('demoData/lsvm-ovarian-model.csv') as csv_file:
...
with open('demoData/lsvm-ovarian-check.csv') as csv_file:
	csv_reader = csv.reader(csv_file, delimiter=',')
	check_count = 0
	check = []
	for row in csv_reader:
...
for i in range(check_count):
	with open('demoData/lsvm-ovarian-data/cancerdata' + str(i+1) + '.dat') as csv_file:
		csv_reader = csv.reader(csv_file, delimiter=',')
		input_count = 0
		xitem = []
		colcount = 0
		for col in csv_reader:		
			xitem.append((float(col[0].lstrip())-float(mu[colcount]))/float(sigma[colcount]))
			colcount += 1
	x.append(xitem);
			
print "sample size: " + str(check_count)
...
for j in range (nruns):
	...
	for i in np.random.permutation(range(len(x)))[0:nquery]:
                if count%10 == 0: print "query " + str(count)
                ...
                if sign(result1)!=check[i]:   #note check data is inverted!
