...
with open('demoData/lsvm-credit-model.csv') as csv_file:
...
# load the inputs
with open('demoData/lsvm-credit-input.csv') as csv_file:
	csv_reader = csv.reader(csv_file, delimiter=',')
	input_count = 0
	x = []
	for row in csv_reader:
...
with open('demoData/lsvm-credit-check.csv') as csv_file:
	csv_reader = csv.reader(csv_file, delimiter=',')
	check_count = 0
	check = []
	for row in csv_reader:
...
for j in range (nruns):
	...
	for i in np.random.permutation(range(len(x)))[0:nquery]:
        ...
        ax.set_ylabel("True")
        ax.set_title("Confusion Matrix for Credit LSVM Prediction pass "+str(j))
        fig.tight_layout()
